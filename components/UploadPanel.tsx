      ...
      <div className="border-t border-slate-800 pt-3">
        <div className="mb-2 flex items-center justify-between gap-2">
          ...
          <button
            ...
            className="rounded-md border border-red-500/60 px-2 py-0.5 text-[11px] font-medium text-red-400 transition hover:bg-red-500/10 disabled:cursor-not-allowed disabled:opacity-50"
          >
            Clear all
          </button>
        </div>
        ...
        {documents.length > 0 && (
          <ul className="flex max-h-40 flex-col gap-2 overflow-y-auto pr-1">
            {documents.map((doc) => (
              <li
                key={doc.id}
                className="group flex items-start justify-between gap-2 rounded-md border border-slate-800 bg-slate-900/70 px-2 py-1.5"
              >
                <div className="flex-1">
                  ...
                  <p className="text-[11px] text-slate-200">{doc.preview}</p>
                </div>
                <button
                  type="button"
                  onClick={() => handleDeleteDocument(doc.id)}
                  className="mt-0.5 inline-flex h-6 shrink-0 items-center justify-center rounded-md border border-slate-700 px-1.5 text-[10px] font-medium text-slate-300 opacity-60 transition hover:border-red-500 hover:text-red-400 hover:opacity-100"
                >
                  Delete
                </button>
              </li>
